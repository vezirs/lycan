...
export default function Home() {
  ...
  const [convId, setConvId] = useState('00000000-0000-0000-0000-000000000200');
  const [input, setInput] = useState('');
  const [token, setToken] = useState(null);
  const socketRef = useRef(null);

  useEffect(() => {
    const stored = typeof window !== 'undefined' ? localStorage.getItem('lycan_token') : null;
    if (stored) setToken(stored);
  }, []);

  useEffect(() => {
    if (!token) return;
    const socket = io('http://localhost:4000/lycan', { auth: { token } });
    ...
    <div style={{ padding: 20, fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      {!token && (
        <div style={{ marginBottom: 12 }}>
          <p>Please login at <a href="/login">/login</a> to connect as an agent.</p>
        </div>
      )}
